"use strict";
/*
interface EpayException extends Error {
    new(message?: string, stack?: unknown, status?: string): EpayException;
    (message?: string, stack?: unknown, status?: string): EpayException;
    status: string
    isEpayException: true;
    readonly prototype: EpayException;
}
*/
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.PaymentException = void 0;
var PaymentException = /** @class */ (function (_super) {
    __extends(PaymentException, _super);
    function PaymentException(message, errors, status) {
        var _this = _super.call(this, message) || this;
        _this.errors = errors;
        _this.status = status;
        Object.setPrototypeOf(_this, PaymentException.prototype);
        return _this;
    }
    return PaymentException;
}(Error));
exports.PaymentException = PaymentException;
